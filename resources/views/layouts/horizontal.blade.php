...
<head>
    ...
    <style>
        ...
        .bold-blue-text {
            font-weight: bold;
            color: #25396f; /* Warna yang diinginkan */
        }

        .navbar-nav .nav-item {
            margin-right: 5px; /* Atur jarak antara setiap item menu */
        }
                                        ...
                                        <li class="nav-item dropdown">
                                            <a class="nav-link active dropdown-toggle text-gray-600" href="#" data-bs-toggle="dropdown" data-bs-display="static" aria-expanded="false">
                                                ...
                                                @if(auth()->user()->unreadNotifications->count() > 0)
                                                    <span class="badge badge-notification bg-danger me-2">{{ auth()->user()->unreadNotifications->count() }}</span>
                                                @endif
                                            </a>
                                            <ul class="dropdown-menu dropdown-menu-end notification-dropdown" aria-labelledby="dropdownMenuButton">
                                                ...
                                                <div class="dropdown-notification-scroll">
                                                    @if(auth()->user()->notifications->isEmpty())
                                                        <li>
                                                            <p class="text-center py-2 mb-0 text-black-50">Tidak terdapat notifikasi</p>
                                                        </li>
                                                    @else
                                                        @foreach (auth()->user()->notifications as $notification)
                                                            <li class="dropdown-item notification-item {{ $notification->read_at ? 'read-notification' : 'unread-notification' }}">
                                                                <a class="d-flex align-items-center" href="{{ $notification->data['url'] }}" data-notification-id="{{ $notification->id }}">
                                                                    <div class="notification-text ms-3">
                                                                        <p class="notification-title font-bold mb-1 text-sm text-black">{{ $notification->data['title'] }}</p>
                                                                        <p class="notification-subtitle font-thin text-sm mb-0 text-black-50">{{ $notification->data['message'] }}</p>
                                                                        <div class="notification-meta d-flex justify-content-between">
                                                                            <p class="small mb-0 text-black-50">{{ $notification->created_at->diffForHumans() }}</p>
                                                                        </div>
                                                                    </div>
                                                                </a>
                                                            </li>
                                                ...
                                                <li>
                                                    <p class="text-center py-2 mb-0">Notifikasi dengan highlight adalah yang belum terbaca.</p>
                                                </li>
                                            </ul>
                                        </li>
                                        <li class="nav-item dropdown">
